    ...
    <div className="relative">
      ...
      >
        <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l3-8H6.4M7 13L5.4 5M7 13l-2 9m12-9l2 9M10 21a1 1 0 100-2 1 1 0 000 2zm8 0a1 1 0 100-2 1 1 0 000 2z" />
        </svg>
        {cartCount > 0 && (
            ...
            <div className="max-h-80 overflow-auto">
              {items.length === 0 ? (
                ...
                <div className="divide-y">
                  {items.map((item) => (
                    <div key={item.productoId} className="flex items-center gap-4 p-4">
                        ...
                        <div className="mt-1 flex items-center gap-3">
                          <div className="inline-flex items-center rounded-md border">
                            <button
                              ...
                              onClick={() => update(item.productoId, Math.max(1, item.cantidad - 1))}
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                update(
                                  item.productoId,
                                  Math.max(1, Math.min(Number(e.target.value || 1), item.stock ?? 9999))
                                )
                              }
                              aria-label="Cantidad"
                            />
                            <button
                              className="px-2 py-1 text-sm hover:bg-accent"
                              onClick={() => update(item.productoId, Math.min(item.cantidad + 1, item.stock ?? 9999))}
