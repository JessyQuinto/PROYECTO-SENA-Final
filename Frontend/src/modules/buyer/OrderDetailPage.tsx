...
const OrderDetailPage: React.FC = () => {
  ...
  const downloadPdf = async () => {
    ...
    const canvas = await html2canvas(rootRef.current, { scale: 2, backgroundColor: '#ffffff' });
  ...
  const load = async () => {
    ...
    const [{ data: o }, { data: s }] = await Promise.all([
      supabase.from('orders').select('id, invoice_number, total, estado, created_at, order_items(id,producto_nombre,cantidad,precio_unitario,subtotal,producto_imagen_url)').eq('id', id).maybeSingle(),
      supabase.from('order_shipping').select('*').eq('order_id', id).maybeSingle(),
    ]);
    setOrder(o); setShipping(s);
    setLoading(false);
  };
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="heading-lg flex items-center gap-3">
            <Icon category="Pedidos" name="MaterialSymbolsOrdersOutlineRounded" className="w-8 h-8" />
        ...
        <div className="flex items-center gap-2">
          <span className={`badge capitalize ${badgeClass(order.estado)}`}>{order.estado}</span>
          <button className="btn btn-secondary flex items-center gap-2" onClick={downloadPdf}>
              ...
              <div className="flex items-center justify-between">
                {STATUS_STEPS.map((s, idx) => (
                  <div key={s.key} className="flex-1 flex items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white ring-2 ring-white shadow ${idx <= currentIdx ? 'bg-green-500' : 'bg-gray-300'}`}>{idx+1}</div>
                    {idx < STATUS_STEPS.length-1 && <div className={`h-1 flex-1 mx-1 rounded ${idx < currentIdx ? 'bg-green-500' : 'bg-gray-300'}`}></div>}
                  </div>
                ))}
              </div>
                    ...
                    <tr key={it.id} className="border-b">
                      <td className="py-3 pr-4">
                        <div className="flex items-center gap-3">
                          {it.producto_imagen_url && <img src={it.producto_imagen_url} alt={it.producto_nombre} className="w-10 h-10 object-cover rounded" />}
          ...
          <div className="card-body space-y-2">
            ...
            <div className="mt-2">
              ...
              {shipping ? (
                ...
                <div className="text-sm text-gray-500">Sin información de envío</div>
              )}
              <p className="text-xs text-gray-500 mt-2">Impuestos incluidos donde aplique</p>
            </div>
          </div>
        </div>
      </div>
