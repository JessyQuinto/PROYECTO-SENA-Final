          ...
          <div className="max-w-md mx-auto">
            <Card className="card-hover">
              <CardContent className="p-8 text-center">
                ...
                <Link to="/productos" className="btn btn-primary flex items-center gap-2 mx-auto text-lg px-8 py-3">
                  <Icon category="Catálogo y producto" name="LineMdSearch" className="w-5 h-5" />
                  Explorar productos
                </Link>
              </CardContent>
            </Card>
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-gray-900 mb-1 truncate">{i.nombre}</h3>
                          <p className="text-lg font-bold text-green-600">${i.precio.toLocaleString()}</p>
                          {i.stock !== undefined && (
                            <p className="text-sm text-gray-500">
                              Stock disponible: {i.stock} unidad{i.stock > 1 ? 'es' : ''}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="inline-flex items-center border-2 border-gray-200 rounded-lg overflow-hidden">
                          <button 
                            ...
                            onClick={() => update(i.productoId, Math.max(1, i.cantidad-1))}
                            disabled={i.cantidad <= 1}
                          >
                            <Icon category="Catálogo y producto" name="WhhArrowdown" className="w-4 h-4" />
                          </button>
                          <input 
                            type="number" 
                          ...
                          <button 
                            ...
                            className="px-3 py-2 text-sm hover:bg-gray-50 flex items-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed" 
                            onClick={() => update(i.productoId, Math.min(i.cantidad+1, i.stock ?? 9999))}
                            disabled={i.cantidad >= (i.stock ?? 9999)}
                          >
                            <Icon category="Catálogo y producto" name="WhhArrowup" className="w-4 h-4" />
                          </button>
                        </div>
                        ...
                        <div className="text-right min-w-[80px]">
                          ...
                          <p className="font-semibold text-gray-900">${(i.precio * i.cantidad).toLocaleString()}</p>
                        </div>
              ...
              <Card className="card-hover sticky top-8">
                <CardContent className="p-6 space-y-6">
                  ...
                  <div className="text-center pb-4 border-b">
                    ...
                    <p className="text-sm text-gray-600">Revisa tu pedido antes de continuar</p>
                  </div>

                  {/* Lista de productos */}
                  <div className="space-y-3 max-h-48 overflow-y-auto">
                    {items.map(i => (
                      <div key={i.productoId} className="flex items-center justify-between text-sm">
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-gray-900 truncate">{i.nombre}</p>
                          <p className="text-gray-500">Cantidad: {i.cantidad}</p>
                        </div>
                        <span className="font-semibold text-gray-900 ml-2">${(i.precio * i.cantidad).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="text-center pt-4 border-t">
                    <div className="flex items-center justify-center gap-2 text-green-600 mb-2">
                      <Icon category="Estados y Feedback" name="MdiShieldCheck" className="w-4 h-4" />
                      <span className="text-sm font-medium">Compra 100% segura</span>
                    </div>
                    <p className="text-xs text-gray-500">
                      Tus datos están protegidos y tu pedido es procesado de forma segura
                    </p>
                  </div>
                </CardContent>
              </Card>
