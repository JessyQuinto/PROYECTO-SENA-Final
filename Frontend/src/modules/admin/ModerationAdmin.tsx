...
const ModerationAdmin: React.FC = () => {
  ...
  const setEstadoProducto = async (id: string, nuevo: ProductoEstado) => {
    ...
    setItems((list) => list.map((p) => (p.id === id ? { ...p, estado: nuevo } : p)));
  };

  return (
    <AdminLayout title="Moderación de Productos" subtitle="Activa, desactiva o bloquea productos que infringen reglas">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <input className="input-hero max-w-md" placeholder="Buscar por nombre o descripción..." value={query} onChange={(e) => setQuery(e.target.value)} />
          <select className="form-select w-40" value={estado} onChange={(e) => setEstado(e.target.value as any)}>
                ...
                <div key={p.id} className="card card-hover overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <span className={`badge ${p.estado==='activo'?'badge-success':p.estado==='inactivo'?'badge-secondary':'badge-danger'}`}>{p.estado}</span>
                      <div className="space-x-2">
                        <button className="btn btn-outline btn-sm" onClick={() => setEstadoProducto(p.id, 'activo')}>Activar</button>
                        <button className="btn btn-outline btn-sm" onClick={() => setEstadoProducto(p.id, 'inactivo')}>Inactivar</button>
                        <button className="btn btn-danger btn-sm" onClick={() => setEstadoProducto(p.id, 'bloqueado')}>Bloquear</button>
                      </div>
                    </div>
                  </div>
                </div>
