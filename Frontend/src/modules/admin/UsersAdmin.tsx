...
const UsersAdmin: React.FC = () => {
  ...
  const [vendedoresConProductos, setVendedoresConProductos] = useState<Set<string>>(new Set());
    ...
    try {
      ...
      if (user?.role === 'vendedor' && role !== 'vendedor') {
        ...
        if (productos && productos.length > 0) {
          ...
          const errorMessage = `❌ NO SE PUEDE CAMBIAR EL ROL ❌\n\n` +
          ...
          alert(errorMessage);
          (window as any).toast?.error(`No se puede cambiar rol: vendedor tiene ${productos.length} producto(s)`, { role: 'admin', action: 'update' });
          return;
        }
      }
      ...
      if (user?.role === 'admin' && role !== 'admin') {
        const confirmMessage = `⚠️ DEGRADAR ADMINISTRADOR ⚠️\n\n` +
      ...
      if (supaUrl && token) {
        ...
        if (user?.role === 'admin' && role !== 'admin') {
          (window as any).toast?.success(`⚠️ Admin degradado exitosamente a ${role}`, { role: 'admin', action: 'update' });
        } else {
  ...
  const removeUser = async (id: string) => {
    ...
    try {
      ...
      (window as any).toast?.error(e?.message || 'No se pudo eliminar', { role: 'admin', action: 'update' });
    }
  };
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        {isSuperAdmin && (
          <div className="flex items-center gap-2 text-sm text-orange-600 font-medium">
            <Icon category="Administrador" name="MdiShieldCheck" className="w-4 h-4" alt="" />
                  ...
                  <tr key={u.id} className="border-t">
                    ...
                    <td className="py-2 pr-4">
                      {canShowButton(u, 'changeRole') ? (
                        <div className="flex flex-col gap-1">
                          ...
                          {u.role === 'admin' && currentUser?.email === SUPER_ADMIN_EMAIL && (
                            <span className="text-xs text-orange-600 font-medium">
                              ⚠️ Super-admin: Puedes degradar
                            </span>
                          )}
                          {u.role === 'vendedor' && vendedoresConProductos.has(u.id) && (
                        ...
                        <div className="flex flex-col gap-1">
                          <span className={`badge ${u.role === 'admin' ? 'badge-error' : u.role === 'vendedor' ? 'badge-warning' : 'badge-info'}`}>
                            {u.role || 'comprador'}
                          </span>
                          {u.role === 'admin' && currentUser?.email !== SUPER_ADMIN_EMAIL && (
                      ...
                      {u.role === 'vendedor' ? (
                        ...
                        <div className="flex items-center gap-2">
                          <span className="badge badge-info flex items-center gap-1">
                            <Icon category="Usuario" name="IconamoonProfileFill" className="w-3 h-3" alt="" />
                            Activo
                          </span>
                        </div>
                      ) : u.role === 'admin' ? (
                    ...
                    <td className="py-2 pr-4">
                      {canShowButton(u, 'blockActions') ? (
                        ...
                        <span className={`badge ${u.bloqueado ? 'badge-error' : 'badge-success'}`}>
