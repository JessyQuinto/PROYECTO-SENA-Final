...
const AdminDashboard: React.FC = () => {
  ...
  const approveVendor = async (vendorId: string) => {
    try {
      ...
      try {
        ...
        console.warn(
          '[notify] fallo al notificar aprobación (continuando):',
          e
        );
      }
  ...
  const rejectVendor = async (vendorId: string) => {
    try {
      ...
      // Si el usuario rechazado está actualmente logueado, refrescar su perfil
      try {
        const currentSession = (await supabase.auth.getSession()).data.session;
        if (currentSession?.user?.id === vendorId) {
          // El usuario rechazado está logueado, refrescar su perfil
