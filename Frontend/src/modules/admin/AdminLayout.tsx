  ...
  const nav = [
    {
      ...
      items: [
        ...
        { to: '/admin/usuarios', label: 'Usuarios', icon: (
          <Icon category="Administrador" name="TablerUsers" className="w-4 h-4" />
        )},
        { to: '/admin/moderacion', label: 'Moderación', icon: (
          <Icon category="Administrador" name="FluentGavel32Filled" className="w-4 h-4" />
        )},
      ]
    },
    {
      ...
      items: [
        { to: '/admin/metricas', label: 'Métricas', icon: (
          <Icon category="Administrador" name="SimpleIconsGoogleanalytics" className="w-4 h-4" />
        )},
        { to: '/admin/auditoria', label: 'Auditoría', icon: (
          <Icon category="Administrador" name="LucideFileClock" className="w-4 h-4" />
        )},
      ]
    },
    {
    ...
    <div className="container py-8">
      <div className="mb-6">
        <h1 className="heading-lg mb-1 flex items-center gap-3">
          <Icon category="Administrador" name="MdiShieldCheck" className="w-8 h-8" />
          {title}
        </h1>
        {subtitle && <p className="text-gray-600 flex items-center gap-2">
          <Icon category="Estados y Feedback" name="TypcnInfoLarge" className="w-4 h-4" />
          {subtitle}
        </p>}
      </div>
              ...
              {nav.map((group) => (
                <div key={group.group} className="py-2">
                  <div className="px-4 py-2 text-xs font-semibold tracking-wide uppercase text-gray-500 flex items-center gap-2">
                    <Icon category="Navegación principal" name="MdiGrid" className="w-3 h-3" />
                    {group.group}
                  </div>
                  <nav className="px-2 pb-2 space-y-1">
                    {group.items.map((item) => (
                      ...
                      >
                        ...
                        {item.to === '/admin' && (
                          <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">default</span>
                        )}
                      </NavLink>
                    ))}
                  </nav>
