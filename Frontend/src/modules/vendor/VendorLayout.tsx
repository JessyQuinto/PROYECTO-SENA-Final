...
const VendorLayout: React.FC<{ title: string; subtitle?: string; children: React.ReactNode }> = ({ title, subtitle, children }) => {
  const nav = [
    {
      group: 'Gestión',
      items: [
        { to: '/vendedor', label: 'Resumen', icon: (
    ...
    },
    {
      group: 'Sistema',
      items: [
        { to: '/vendedor#config', label: 'Configuración', icon: (
    ...
    <div className="container py-8">
      <div className="mb-6">
        <h1 className="heading-lg mb-1 flex items-center gap-3">
          <Icon category="Vendedor" name="MaterialSymbolsDashboard" className="w-8 h-8" />
          {title}
        </h1>
