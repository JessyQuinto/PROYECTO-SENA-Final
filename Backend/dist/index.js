...
app.post('/auth/post-signup', async (req, res, next) => {
...
app.post('/admin/create-user', rateLimit, requireAdminJwt, async (req, res, next) => {
    ...
    try {
        ...
        if (!userId)
            return res.status(500).json({ error: 'No se obtuvo user.id' });
...
app.post('/rpc/crear_pedido_demo', async (req, res, next) => {
...
app.post('/rpc/crear_pedido', async (req, res, next) => {
    ...
    try {
        ...
        if (errPedido) {
            ...
            return res.status(400).json({ error: errPedido.message, code: errPedido.code, details: errPedido.details, hint: errPedido.hint });
        }
...
app.post('/payments/simulate', async (req, res, next) => {
...
// ==============================
app.post('/orders/:id/cancel', rateLimit, requireUser, async (req, res, next) => {
    try {
        const authHeader = req.headers.authorization || '';
        const supabaseUser = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY, {
...
app.post('/orders/:id/delivered', rateLimit, requireUser, async (req, res, next) => {
    try {
        const authHeader = req.headers.authorization || '';
        const supabaseUser = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY, {
            global: { headers: { Authorization: authHeader } }, auth: { autoRefreshToken: false, persistSession: false }
...
app.post('/order-items/:id/shipped', rateLimit, requireUser, async (req, res, next) => {
    try {
        const authHeader = req.headers.authorization || '';
        const supabaseUser = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY, {
            global: { headers: { Authorization: authHeader } }, auth: { autoRefreshToken: false, persistSession: false }
